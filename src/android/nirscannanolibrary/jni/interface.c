 ...
 */
JNIEXPORT jobject JNICALL Java_com_ubi_nirscannanolibrary_KSTNanoSDK_dlpSpecScanInterpReference(JNIEnv* env, jobject thiz,jbyteArray data, jbyteArray coeff, jbyteArray matrix){
    ...
    jclass clazz = (*env)->FindClass(env, "com/ubi/nirscannanolibrary/KSTNanoSDK$ScanResults");
    if(clazz == NULL){
 ...
 * appropriate data type inside the application
 */
JNIEXPORT jobject JNICALL Java_com_ubi_nirscannanolibrary_KSTNanoSDK_dlpSpecScanReadConfiguration(JNIEnv* env, jobject thiz,jbyteArray data){
    ...
    if(dlpspec_is_slewcfgtype((void *)bufferPtr,lengthOfArray)){
        ...
        jclass clazz = (*env)->FindClass(env, "com/ubi/nirscannanolibrary/KSTNanoSDK$ScanConfiguration");
        if(clazz == NULL){
