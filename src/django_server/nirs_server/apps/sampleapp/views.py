...
def results(request):
    ...
    if (mode is None) or (device_id is None) or (transaction_number is None):
    ...
    if mode == "test":
        ...
        UbiNIRSAppMetadata.nirs_estimator.add_posted_data(
            wavelength, intensity_spectrum, reference_spectrum, result_label, "test", device_id, transaction_number)
        ...
        try:
            ...
            UbiNIRSAppMetadata.nirs_estimator.add_posted_data(
                wavelength, intensity_spectrum, reference_spectrum, label, "train", device_id, transaction_number)
...
def feedback(request):
    ...
    # No such parameter.
    if (answer is None) or (mode is None) or (transaction_number is None) or (device_id is None) or (label is None):
